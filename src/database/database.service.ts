...
export class DatabaseService {
    ...
    async executeQuery(queryText: string, values: any[] = []): Promise<any[]> {
        ...
        const cachedResponse = await this.cacheManager.get(preprocessedQuery);
        if (cachedResponse) {
            this.logger.debug(`Cache hit for query: ${preprocessedQuery} (${values})`);
            return cachedResponse as Promise<any[]>;
        }
